...
/* Private typedef -----------------------------------------------------------*/
typedef struct{
  uint16_t  CustomMy_HsHdle;                    /**< My_Humidity_Server handle */
  uint16_t  CustomHum_CHdle;                  /**< My_Humidity_Char handle */
...
void SVCCTL_InitCustomSvc(void)
{
   ...
   * service_max_attribute_record = 1 for My_Humidity_Server +
   *                                2 for My_Humidity_Char +
   *                              = 3
   *
  ...
  ret = aci_gatt_add_service(UUID_TYPE_128,
                             ...
                             PRIMARY_SERVICE,
                             max_attr_record,
                             &(CustomContext.CustomMy_HsHdle));
  if (ret != BLE_STATUS_SUCCESS)
  {
    APP_DBG_MSG("  Fail   : aci_gatt_add_service command: My_HS, error code: 0x%x \n\r", ret);
  }
  else
  {
    APP_DBG_MSG("  Success: aci_gatt_add_service command: My_HS \n\r");
  }
  ...
  ret = aci_gatt_add_char(CustomContext.CustomMy_HsHdle,
                          ...
                          &(CustomContext.CustomHum_CHdle));
  if (ret != BLE_STATUS_SUCCESS)
  {
    APP_DBG_MSG("  Fail   : aci_gatt_add_char command   : HUM_C, error code: 0x%x \n\r", ret);
  }
  else
  {
    APP_DBG_MSG("  Success: aci_gatt_add_char command   : HUM_C \n\r");
  }
  ...
  switch (CharOpcode)
  {
    ...
    case CUSTOM_STM_HUM_C:
      ret = aci_gatt_update_char_value(CustomContext.CustomMy_HsHdle,
                                       CustomContext.CustomHum_CHdle,
                                       0, /* charValOffset */
                                       SizeHum_C, /* charValueLen */
                                       (uint8_t *)  pPayload);
      if (ret != BLE_STATUS_SUCCESS)
      {
        APP_DBG_MSG("  Fail   : aci_gatt_update_char_value HUM_C command, result : 0x%x \n\r", ret);
      }
      else
      {
